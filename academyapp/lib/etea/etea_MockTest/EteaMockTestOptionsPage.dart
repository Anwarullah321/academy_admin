...
import 'EteaMockTestPage.dart';
import 'history/MockTestHistoryPage.dart';

class MockTestOptionsPage extends StatefulWidget {
  final MCQService mcqService;

  MockTestOptionsPage({required this.mcqService});

  @override
  _MockTestOptionsPageState createState() => _MockTestOptionsPageState();
}

class _MockTestOptionsPageState extends State<MockTestOptionsPage> {
  ...
  Future<void> _startTest() async {
    ...
    try {
      List<MCQ> mockTestMCQs = await _getAllMCQs();

      if (!mounted) return;

      int expectedTotal = 100; // Total MCQs expected
      if (mockTestMCQs.length != expectedTotal) {
        setState(() {
          _errorMessage =
              'Incorrect number of MCQs loaded (Expected: $expectedTotal, Got: ${mockTestMCQs.length}).\n\n' +
                  _subjectStatus.entries
                      .map((e) => '${e.key}: ${e.value}')
                      .join('\n');
        });
        return;
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => EteaMockTestPage(mcqs: mockTestMCQs),
        ),
      );
    } catch (e) {
      if (!mounted) return;
            ...
            children: [
              ...
              _buildGradientButton('View History', () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => MockTestHistoryPage(),
                  ),
                );
              }),
            ],
