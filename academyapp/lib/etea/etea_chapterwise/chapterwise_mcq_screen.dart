...
import 'EteaDetailResultPage.dart';


class EteaChapterwiseQuizPage extends StatefulWidget {
  final String subject;
  final String chapter;
  final List<MCQ> mcqs;

  EteaChapterwiseQuizPage({required this.subject, required this.chapter, required this.mcqs});

  @override
  _EteaChapterwiseQuizPageState createState() => _EteaChapterwiseQuizPageState();
}

class _EteaChapterwiseQuizPageState extends State<EteaChapterwiseQuizPage> {
  final MCQService _eteachapterwisemcqService = MCQService();
  List<MCQ> _mcqs = [];
  int _currentIndex = 0;
  ...
  Future<String> _generateTaskName() async {
    ...
    final key = '${widget.subject}_${widget.chapter}_eteaobjective';
  ...
  void _showDetailedResults(int correctAnswers, int totalQuestions) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => EteaDetailedResultsPage(
          mcqs: _mcqs,
  ...
  Widget build(BuildContext context) {
    if (_isLoading) {
      return Scaffold(
        appBar: AppBar(
            title: Text('Etea - ${widget.subject} - ${widget.chapter}',
