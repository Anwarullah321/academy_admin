...
VG!g[,4griq0aZzqHWPnNu_{:v1;YW''';
  ...
  static String decryptUnmaskAndUnshuffleKey(String protectedRsaKey, String keyType) {
    String aesKey = unshuffleString(
        unmaskString(
            NexaSoftLicenseGenerator.decompress(
...
//***These two function will  used to encrypt aes key used to encrypt YAML files */
  static Future<String> protectAes({required String aesKey, required String encRsaPublicKey})async{
    ...
    String encryptedAesKey = await NexaSoftLicenseGenerator.encryptString(data: masked, publicKey: rsaPublic);
  ...
  static Future<String> unProtectAes({required String encAesKey, required String encRsaPrivateKey})async{
    ...
    String decryptedAesKey = await NexaSoftLicenseGenerator.decryptString(
        encryptedText: encAesKey,
...
//***** */
  ...
  static String shuffleString(String input, String keyType) {
    ...
    List<int> order = [];
    if (keyType == "private") {
      order = NexaSoftLicenseGenerator.decompress(base64.decode(soPriv)).replaceAll("[", "").replaceAll("]", "")
          .split(",")
          .map((e) => int.parse(e))
          .toList();
    } else if (keyType == "aes") {
      order = NexaSoftLicenseGenerator.decompress(base64.decode(soAes)).replaceAll("[", "").replaceAll("]", "")
          .split(",")
          .map((e) => int.parse(e))
          .toList();
    } else {
      order = NexaSoftLicenseGenerator.decompress(base64.decode(soPub)).replaceAll("[", "").replaceAll("]", "")
          .split(",")
          .map((e) => int.parse(e))
          .toList();
    }
