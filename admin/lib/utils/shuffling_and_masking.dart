...
//   static Future<String> protectAes({required String aesKey, required String encRsaPublicKey})async{
//     String rsaPublic = decryptUnmaskAndUnshuffleKey(encRsaPublicKey, 'public');
//     String shuffled = shuffleString(aesKey, 'aes');
//     String masked = maskString(shuffled, mask);
//     String encryptedAesKey = await NexaSoftRollNumberGenrator.encryptString(data: masked, publicKey: rsaPublic);
//     return encryptedAesKey;
//   }

//     static Future<String> unProtectAes({required String encAesKey, required String encRsaPrivateKey})async{
//     String rsaPrivate = decryptUnmaskAndUnshuffleKey(encRsaPrivateKey, 'private');
//     String decryptedAesKey = await NexaSoftRollNumberGenrator.decryptString(encryptedText: encAesKey, privateKey: rsaPrivate);
...
//***These two function will  used to encrypt aes key used to encrypt YAML files */
  static Future<String> protectAes({required String aesKey, required String rsaPublicKey})async{
    String shuffled = shuffleString(aesKey, 'aes');
    String masked = maskString(shuffled, mask);
    String encryptedAesKey =  NexaSoftRollNumberGenrator.encryptString(data: masked, publicKey: rsaPublicKey);
    return encryptedAesKey;
  }

  static String unProtectAes({required String encAesKey, required String rsaPrivateKey}){
    String decryptedAesKey =  NexaSoftRollNumberGenrator.decryptString(encryptedText: encAesKey, privateKey: rsaPrivateKey);
    String unMasked = unmaskString(decryptedAesKey, mask);
    String aesKey = unshuffleString(unMasked, 'aes');
    return aesKey;
  }
...
//***** */
  ...
  static String shuffleString(String input, String keyType) {
   ...
   List<int> order = [];
    if (keyType == "private") {
      order = NexaSoftRollNumberGenrator.decompress(base64.decode(soPriv)).replaceAll("[", "").replaceAll("]", "")
          .split(",")
          .map((e) => int.parse(e))
          .toList();
    } else if (keyType == "aes") {
      order = NexaSoftRollNumberGenrator.decompress(base64.decode(soAes)).replaceAll("[", "").replaceAll("]", "")
          .split(",")
          .map((e) => int.parse(e))
          .toList();
    } else {
      order = NexaSoftRollNumberGenrator.decompress(base64.decode(soPub)).replaceAll("[", "").replaceAll("]", "")
          .split(",")
          .map((e) => int.parse(e))
          .toList();
    }
  ...
  // static List<int> shuffleOrder = [];

  // static String shuffleString(String input) {
  //   List<String> characters = input.split('');
  //   List<int> indices = List.generate(characters.length, (index) => index);

  //   // Fisher-Yates Shuffle to get the shuffle order
  ...
  //   // Store the shuffle order for unshuffling
  //   shuffleOrder = List.from(indices);
  //   print(shuffleOrder);
  //   return characters.join('');
  // }

  // static String unshuffleString(String shuffledInput) {
  //   List<String> characters = shuffledInput.split('');
  //   List<String> original = List.filled(characters.length, '');

  //   // Use the stored shuffle order to place characters back to their original positions
  //   for (int i = 0; i < shuffleOrder.length; i++) {
  //     int originalIndex = shuffleOrder[i];
  //     original[originalIndex] = characters[i];
  //   }
